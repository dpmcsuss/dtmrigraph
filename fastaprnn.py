...
class FAKPseudoNeighbor(FAKNeighborsClassifier):
    ...
    def kneighbors(self, X, idx=None, n_neighbors=None, return_distance=True):
        ...
        if gneighbors.shape[0]>self._y.shape[0]-n_neighbors:
            print "WOAH "+repr( gneighbors.shape[0])+" NEIGHBORS!!! Giving UP :( Return random stuff"
            return np.zeros(n_neighbors), np.array(sample(np.arange(self._y.shape[0]),n_neighbors))
        ...
        notGneighbor = np.equal(np.in1d(ind.flat, gneighbors,True),False)[None,:]
...
#    a, axis = _chk_asarray(a, axis)
    ...
    return np.array([invDict[m] for m in mostfrequent]),scorecount[np.arange(n).astype(int),mostfrequent.astype(int)]
