...
def knn_experiments_no_clique(
    brainfiles, embedDir, graphDir, lccDir, roiDir,
    kRange, dRange,trainFra, testSize, nmc,leftRight=False, savefn=None):
...
def knn_experiments_compare(
    brainfiles, embedDir, graphDir, lccDir, roiDir,
    kRange, dRange,fracRange, nmc, testSize=None,leftRight=False, savefn=None):
    """
    ...
    for bfn in brainfiles:
        ...
        for frac in fracRange:
            ...
            for train, test in lfo:
                resC = np.recarray(nrec,dtype=[('bfn',object),('k',int),('d',int),('frac',float),('Lhat',float),('test')])
...
def all_whole_brain_results(bfiles, mrnDict, kRange,dRange,nfolds,savefn,testSz):
    ...
    for bfn in bfiles:
        ...
        for d in dRange:
            ...
            for (train,test),fold in izip(crossval,xrange(nfolds)):
                fpnnList[fold] = train_fpnn(x[train,:],y[train],G[train,:],kMax)
            ...
            for k in kRange:
                ...
                if savefn is not None:
                    np.savez(savefn+'{}_k={}_d={}_folds={}.npz'.format(bfn, k, d,nfolds),yN,yC,LN,LC)
...
def knn_error_plot(bfiles, kRange,dRange,nfolds,savefn):
    ...
    for k,j in zip(kRange,xrange(nk)):
        ...
        for d,i in zip(dRange,xrange(nd)):
            LN[i,:] = np.concatenate(
                [np.load(savefn+'{}_k={}_d={}_folds={}.npz'.format(bfn, k, d,nfolds))['arr_2']
                                    for bfn in bfiles])

            LC[i,:] = np.concatenate(
                [np.load(savefn+'{}_k={}_d={}_folds={}.npz'.format(bfn, k, d,nfolds))['arr_3']
                                    for bfn in bfiles])
...
def get_cv_neighbors(x,y,G,crossval,k,d, permute=False):
    ...
    for fold,(train,test) in enumerate(crossval):
        ...
        neighC[test] = np.squeeze([train[fpnn.kneighbors(x[idx,:],return_distance=False)]
                                for idx in test]).reshape(neighC[test].shape)

        print "Getting Non-Graph-Neighbor Neighbors"

        neighN[test] = np.squeeze([train[fpnn.kneighbors(x[idx,:],idx,return_distance=False)]
...
def get_whole_brain_results(x,y,G,k,d,crossval,permute=False):  #bfn, embedDir, graphDir, lccDir, roiDir,k,d,nfolds):
    ...
    for train, test in crossval:
        ...
        # Get Predicted Labels using No Cliques
        yHatN[test] = np.array([fpnn.predict(x[idx,:],idx) for idx in test]).flatten().astype(yHatN.dtype)
        LhatN[fold] = np.mean(np.not_equal(y[test],yHatN[test]))
        #print "No Clique Error = {}".format(LhatN[fold])
        
        # and using cliques, ie no idx ............. v ... there
        yHatC[test] = np.array([fpnn.predict(x[idx,:]) for idx in test]).flatten().astype(yHatN.dtype)
