...
import numpy as np
from scipy.stats import norm, nanmean
...
def smooth_err_rate_cv(x,err,xRange,bw):
    ...
    cv = cross_validation.ShuffleSplit(n,n_iterations=n_iter,test_fraction=.1,indices=True)
    ...
    for train_test in cv:
        ...
        while k < 5:
            fMin = np.array([gaussian_average(x,err,xx,bw,False) for xx,bw in xRange])
            fMax = np.array([gaussian_average(x,err,xx,bw,False) for xx in xRange])
...
def plot_smoothErr(x,y,xRange,bwRange,c=None):
    ...
    for i,bw in enumerate(bwRange):
        ...
        plt.fill_between(xRange, smoothErr+se,smoothErr-se, color=cbw*.7)
        plt.plot(xRange,smoothErr,color=cbw,linewidth=2,label=r'$\sigma={}$'.format(bw))
...
def get_roi_boundary(roi3d, lccCoord,keepNan = False):
    ...
    if keepNan:
        return np.array([np.any(
            [roi3d.item(*coord)!=roi3d.item(*(coord+n))
             for n in neigh])
                         for coord in lccCoord])
    else:
        return np.array([np.any(
            [roi3d.item(*coord)!=roi3d.item(*(coord+n)) and not np.isnan(roi3d.item(*(coord+n)))
             for n in neigh])
                         for coord in lccCoord])
...
def is_boundary(roi3d, coord):
    ...
    return np.any([roi3d.item(*coord)!=roi3d.item(*(coord+n)) for n in neigh])
...
def dist_to_boundary(distItem,coord):
    ...
    neigh = np.array([[ 1, 0, 0],
                      ...
                      [ 0, 0, 1],
                      [ 0, 0,-1]]);
    return np.nanmin([distItem(*(coord+n))+np.linalg.norm(n) for n in neigh])
...
def rotate_labels(y,y3d,lccCoord,theta,axis,center=None,return3d=False):
    ...
    lccNew = np.round((lccCoord-center).dot(rotate_matrix(axis,theta))+center).astype(int)
